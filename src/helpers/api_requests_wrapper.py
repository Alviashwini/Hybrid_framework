# Help you to make post, put, patch and delete.
import json
import requests
...
def post_request(url,auth,headers,payload,in_json):
    post_response = requests.post(url=url, headers=headers, auth=auth, data=json.dump)
    if in_json is True:
        return post_response.json()
    return post_response

def patch_request(url,auth,headers,payload,in_json):
    patch_response_data = requests.patch(url=url, headers=headers, auth=auth, data=json.dump)
    if in_json is True:
        return patch_response_data.json()
    return patch_response_data
def put_requests(url,auth,headers,payload,in_json):
    put_response_data = requests.put(url=url, headers=headers, auth=auth, data=in_json)
    if in_json is True:
